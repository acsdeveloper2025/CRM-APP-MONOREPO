/**
 * Test NOC Verification Database Insertion
 */

const { Pool } = require('pg');
const { mapNocFormDataToDatabase, validateNocRequiredFields, getNocAvailableDbColumns } = require('./dist/utils/nocFormFieldMapping');
const { detectBusinessFormType } = require('./dist/utils/formTypeDetection'); // Use business detection for NOC
...
async function testNocVerification() {
  console.log('🧪 Testing NOC Verification Database Insertion\n');

  try {
    // Sample comprehensive NOC form data for different form types
    const testCases = [
      {
        name: 'POSITIVE NOC Verification',
        formData: {
          outcome: 'VERIFIED',
          addressLocatable: 'Easy',
          addressRating: 'Excellent',
          nocStatus: 'Available',
          nocType: 'Environmental Clearance',
          nocNumber: 'ENV/2024/001234',
          nocIssueDate: '2024-01-15',
          nocExpiryDate: '2026-01-15',
          nocIssuingAuthority: 'State Pollution Control Board',
          nocValidityStatus: 'Valid',
          metPersonName: 'Project Manager',
          ...
          constructionStatus: 'Foundation Complete',
          ...
          builderRegistrationNumber: 'RERA/2023/ABC001',
          propertyType: 'Residential Apartments',
          documentShownStatus: 'Yes',
          documentType: 'NOC Certificate, Building Plan, RERA Certificate',
          documentVerificationStatus: 'Verified',
          environmentalClearance: 'Available',
          fireSafetyClearance: 'Available',
          pollutionClearance: 'Available',
          waterConnectionStatus: 'Connected',
          electricityConnectionStatus: 'Connected',
          tpcMetPerson1: 'Yes',
          nameOfTpc1: 'Local Resident',
          tpcConfirmation1: 'Positive',
          tpcMetPerson2: 'No',
          nameOfTpc2: '',
          tpcConfirmation2: null,
          locality: 'Industrial Development Area',
          addressStructure: 'Construction Site',
          addressFloor: 'Ground Level',
          ...
          otherObservation: 'All NOC documents verified successfully. Project is progressing as per approved plan.',
          finalStatus: 'Positive',
          remarks: 'NOC verification completed successfully'
        }
      },
      {
        name: 'SHIFTED NOC Verification',
        formData: {
          outcome: 'SHIFTED',
          ...
          metPersonRelation: 'Employee',
          shiftedPeriod: '1.5 years ago',
          currentLocation: 'New Development Site, Sector 45',
          premisesStatus: 'Vacant',
          locality: 'Old Industrial Area',
          addressStructure: 'Abandoned Site',
          addressFloor: 'Ground Level',
          addressStructureColor: 'Faded White',
          doorColor: 'Rusted',
          landmark1: 'Near Old Factory',
          landmark2: 'Behind Closed Mall',
          politicalConnection: 'No',
          dominatedArea: 'No',
          feedbackFromNeighbour: 'NoAdverse',
          infrastructureStatus: 'Poor',
          roadConnectivity: 'Average',
          otherObservation: 'Project has been shifted to new location. Old site is abandoned.',
          finalStatus: 'Negative'
        }
      },
      {
        name: 'UNTRACEABLE NOC Verification',
        formData: {
          outcome: 'UNTRACEABLE',
          contactPerson: 'Local Worker',
          callRemark: 'Did Not Pick Up Call',
          locality: 'Remote Construction Area',
          landmark1: 'Near Highway',
          landmark2: 'Behind Fuel Station',
          dominatedArea: 'No',
          otherObservation: 'Unable to locate the project site or any responsible person',
          finalStatus: 'Negative'
        }
      }
    ];
    ...
    for (const testCase of testCases) {
      ...
      // Detect form type (using business detection for NOC)
      ...
      const mappedFormData = mapNocFormDataToDatabase(testCase.formData);
      console.log(`   Mapped Fields: ${Object.keys(mappedFormData).length}`);

      // Validate required fields
      const validation = validateNocRequiredFields(testCase.formData, formType);
      ...
      const dbInsertData = {
        ...
        customer_email: null,
        full_address: 'Test NOC Project Address, Development Zone',
        ...
        total_images: 5,
        total_selfies: 1,
        remarks: testCase.formData.remarks || `${formType} NOC verification completed`,
      ...
      try {
        ...
        const verifyQuery = `
          SELECT form_type, verification_outcome, met_person_name, locality, 
                 address_locatable, noc_status, noc_type, project_name, builder_name,
                 political_connection, other_observation, final_status
        ...
        console.log(`      NOC Status: ${verifiedRecord.noc_status || 'NULL'}`);
        console.log(`      NOC Type: ${verifiedRecord.noc_type || 'NULL'}`);
    ...
    console.log('📊 NOC Verification Database Summary:\n');
    ...
    const statsQuery = `
      SELECT 
        ...
        COUNT(CASE WHEN noc_status IS NOT NULL THEN 1 END) as has_noc_status,
    ...
    console.log('NOC Form Type Distribution:');
    statsResult.rows.forEach(row => {
      ...
      console.log(`      Has NOC Status: ${row.has_noc_status}/${row.count}`);
      console.log(`      Has Project Name: ${row.has_project_name}/${row.count}`);
      console.log(`      Has Builder Name: ${row.has_builder_name}/${row.count}`);
    });

    console.log('\n✅ NOC verification database insertion testing completed successfully!');
...
// Run the test
testNocVerification().catch(console.error);
