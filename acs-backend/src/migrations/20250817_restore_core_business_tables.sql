...
-- Description: Restores core business tables that were accidentally removed (products, verification_types, client_products, client_verification_types)
-- These are needed for client management and case management, not just rate management
-- Created: 2025-08-17
...
-- Restore client_products table (for client management)
...
-- Restore client_verification_types table (for client management)
CREATE TABLE IF NOT EXISTS "clientVerificationTypes" (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    "clientId" UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
...
-- Create sample client-product relationships
...
-- Create sample client-verification type relationships
INSERT INTO "clientVerificationTypes" ("clientId", "verificationTypeId")
SELECT c.id, vt.id
FROM clients c
CROSS JOIN "verificationTypes" vt
LIMIT 12
ON CONFLICT ("clientId", "verificationTypeId") DO NOTHING;
...
CREATE INDEX IF NOT EXISTS idx_client_verification_types_client_id ON "clientVerificationTypes"("clientId");
CREATE INDEX IF NOT EXISTS idx_client_verification_types_verification_type_id ON "clientVerificationTypes"("verificationTypeId");

-- Add comments
COMMENT ON TABLE products IS 'Core business products offered by the organization';
COMMENT ON TABLE "verificationTypes" IS 'Types of verification services available';
COMMENT ON TABLE pincodes IS 'Postal codes linked to cities';
COMMENT ON TABLE areas IS 'Geographic areas within cities';
COMMENT ON TABLE "clientProducts" IS 'Products available to each client';
COMMENT ON TABLE "clientVerificationTypes" IS 'Verification types available to each client';
...
AND table_name IN ('products', 'verificationTypes', 'pincodes', 'areas', 'clientProducts', 'clientVerificationTypes')
ORDER BY table_name;
